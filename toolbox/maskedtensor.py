...
def from_list(tensor_list, dims, batch_name="B", base_name="N"):
    ...
    for i, tens in enumerate(tensor_list):
        ...
        data_padding = [
            [0, data_size[dim + 1] - tens.size(dim)] for dim in range(n_dim)
        ]
    ...
    for dim, name in enumerate(data.names):
        if dim >= 1 and name:
            mask[name] = torch.zeros(
                (batch_size, data.size(name)),
                names=(batch_name, name),
                dtype=data.dtype,
            )
            for i, tens in enumerate(tensor_list):
...
class MaskedTensor:
    ...
    def __init__(
        self, data, mask, adjust_mask=True, apply_mask=False, copy=False, batch_name="B"
    ):
    ...
    def mask(self):
        ...
        return MaskedTensor(
            self.tensor, self.mask_dict, adjust_mask=False, apply_mask=True, copy=True
        )
    ...
    def __getitem_mt__(self, index):
        ...
        dims_to_squeeze = (
            []
        )  # Keeping track of which dimension should be removed at the end
            ...
            if i < len(index):
                ...
                if (slice_value, int):
                    ...
                    raise NotImplementedError(
                        f"Don't know what to do for slices like this : {slice_value}"
                    )
        ...
        for name, mask in self.mask_dict.items():
            ...
            built_index = [
                index_mask_dict.get(mask_name, slice(None)) for mask_name in mask_names
            ]
            if built_index:
        ...
        return MaskedTensor(
            new_tensor, new_mask_dict, adjust_mask=False, apply_mask=False
        )
    ...
    def permute(self, *dims):
        ...
        return MaskedTensor(
            res_tensor, self.mask_dict, adjust_mask=False, apply_mask=False
        )

    def to(self, *args, **kwargs):
        """Apply the method .to() to both tensor and mask"""
        new_dict = {
            name: mask.to(*args, **kwargs) for name, mask in self.mask_dict.items()
        }
    ...
    def scatter_(self, *args, **kwargs):
        ...
        return MaskedTensor(
            res_tensor, self.mask_dict, adjust_mask=False, apply_mask=True
        )
...
def torch_max(masked_tensor, dim):
    ...
    new_masked_tensor = MaskedTensor(
        max_tensor, masked_tensor.mask_dict, adjust_mask=True, apply_mask=True
    )
...
def dispatch_cat(tensors, dim=0):
    ...
    return tensor.__torch_function__(
        torch.cat, [type(t) for t in tensors], (tensors,), {"dim": dim}
    )
...
def torch_squeeze(inp, dim=None):
    ...
    res_mask_dict = {
        name: elt for name, elt in inp.mask_dict.items() if not (name in popped)
    }
...
def torch_flatten(inp, start_dim=0, end_dim=-1):
    ...
    res_tensor = torch.flatten(
        inp.tensor.rename(None), start_dim=start_dim, end_dim=end_dim
    )
...
def torch_binary_cross_entropy(inp, target, *args, **kwargs):
    ...
    ), "Different shapes not taken into account... You should implement it."
    names = inp.tensor.names
    nameless_tensor = inp.tensor.rename(None)
    target_nameless_tensor = target.tensor.rename(None)
    nameless_res_tensor = F.binary_cross_entropy(
        nameless_tensor, target_nameless_tensor, *args, **kwargs
    )
    if kwargs.get("reduction", None) is None:
        res_tensor = nameless_res_tensor.rename(*names)
        return MaskedTensor(
            res_tensor, inp.mask_dict, adjust_mask=False, apply_mask=True
        )
